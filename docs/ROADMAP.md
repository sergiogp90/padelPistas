...
| **M6 — Datos reales** *(futuro)* | Sustituir el *mock* por una fuente real (una **API propia** o panel de control) implementando la interfaz `DataSource`. Además, **inicializar los avatares a partir de los datos** de cada jugador (p. ej. elegir el diseño de avatar según el género), en lugar de colocarlos con valores fijos. | 💡 Idea |
